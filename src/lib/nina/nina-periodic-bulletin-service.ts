...
export interface NinaBulletinServiceEvents {
...
export class NinaPeriodicBulletinService {
  ...
  private events: NinaBulletinServiceEvents = {}
  ...
  generateBulletin(input: BulletinContextInput, isScheduledTrigger = false): NinaBulletinPayload {
    ...
    if (isShortUpdate) {
      sentences.push(
        'Boletim Nina: Parâmetros estáveis e sem alterações relevantes desde o último boletim.',
      )
    } else {
      sentences.push(
        changes.length > 0 ? `Boletim Nina: ${changes.join('. ')}.` : 'Boletim periódico da Nina.',
      )
    }
   ...
   * - "Nina, me avisa a cada 20 minutos" (e outros números)
   * - "Nina, deixa os boletins mais detalhados" / "mais resumidos"
   * - "Nina, desativa os boletins"
   * Retorna resposta descritiva se for um comando de boletim ou null se não reconhecido.
   */
  parseVoiceCommand(transcript: string): { handled: boolean; replyText: string } | null {
    const raw = transcript.toLowerCase().trim()
    const clean = raw.replace(/^nina[,.\s]*/i, '').trim()
